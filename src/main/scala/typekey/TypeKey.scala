...
case class TypeKey[A](val tag: TypeTag[A]) {
  ...
  lazy val typeArgs: List[TypeKey[_]] = tpe.typeArgs map { tpe => TypeKey(makeTypeTag(tpe, tag.mirror)) }

  /** a copy of this `TypeKey`, but with the [[tag underlying `TypeTag`]]
   * replaced with a copy that has a different mirror.
   *
   * this method is helpful for managing code that crosses ClassLoader
   * boundaries.
   */
  def inMirrorOf(key: TypeKey[_]): TypeKey[A] = TypeKey(tag.in(key.tag.mirror))

  /** if `B` is a lower bound to type `A`, then return `this` cast to
   * `TypeKey[_ >: B]`, wrapped in an option. this method is provided for Scala
   * grammer only, so that you can reuse this type to safely please the compiler
   * into matching types. however, the return value will remain `==` to `this`,
   * so it can be reused as a key into a `TypeKeyMap`. but it will probably not
   * be equal to a `TypeKey[_ >: B]` generated by other means. in short, the
